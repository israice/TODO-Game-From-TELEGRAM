...
const config = require('./delete_task_config');
...
try {
  ...
  if (userData.action === 'delete_task') {
...
(async () => {
  ...
  try {
    ...
    // Step 2: Delete task by index
    console.log(`Deleting task at index ${taskIndex + 1}`);
    ...
    if (taskIndex < 0 || taskIndex >= count) {
      ...
      // Get the delete button within the same li element
      ...
      console.log(`✓ Task #${taskIndex + 1} deleted successfully`);
    }
